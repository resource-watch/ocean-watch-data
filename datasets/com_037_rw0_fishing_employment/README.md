## Fishing Employment Dataset
This file describes the Fishing Employment dataset on [Ocean Watch](https://www.oceanwatchdata.org)

### Function
The number of people employed in natural sectors activities (Agriculture, hunting, forestry, and fishing) in each country or territory, dissagreatted by economic activitiy, sex, and year.

### Source
International Labour Organization

### About the data
- **Geographic coverage:** 214 countries and territories
- **Date of content:** 1992-2020
- **Frequency of updates:** Yearly

<br/>*Learn more about the data [here](https://ilostat.ilo.org/data/#)*

### Download the data
[Employment by sex and status in employment (Classification level 1)](https://www.ilo.org/shinyapps/bulkexplorer52/?lang=en&segment=indicator&id=EMP_TEMP_SEX_ECO_NB_A&ref_area=&sex=SEX_T+SEX_M+SEX_F+SEX_O&classif1=ECO_ISIC4_A+ECO_ISIC3_A+ECO_ISIC3_B&timefrom=1947&timeto=2020) <br>

[Employment by sex and status in employment (Classification level 2)](https://www.ilo.org/shinyapps/bulkexplorer22/?lang=en&segment=indicator&id=EMP_TEMP_SEX_EC2_NB_A&ref_area=&sex=SEX_T+SEX_M+SEX_F+SEX_O&classif1=EC2_ISIC4_A01+EC2_ISIC4_A02+EC2_ISIC4_A03+EC2_ISIC3_A01+EC2_ISIC3_A02+EC2_ISIC3_B05&timefrom=1992&timeto=2020) 


### Pre-processing
The Ocean Watch data team used a [python script]({link-to-script}) to aggregate and upload the selected data to [a table on Carto](https://resourcewatch.carto.com/u/wri-rw/dataset/com_037_rw0_fishing_employment_edit)

### Citation
International Labour Organization. (2020). ILOSTAT database \[database]. Available from https://ilostat.ilo.org/data/.